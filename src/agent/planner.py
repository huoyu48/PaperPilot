...
# Dynamic year strings — current year + previous year.
# _YEAR_NOW is read at call time, not at import time, so it stays correct
# even if the process was started in a different year.
def _years_str() -> str:
...
If "上传/这篇论文" mentioned, include a local_docs sub-query. Append the current+previous year to arxiv queries about recent topics.
...
def _fast_plan(query: str) -> list[SubQuery]:
    ...
    arxiv_q = f"{q} 综述 {years}" if has_latest_intent else f"{q} 研究论文"
    ...
    if has_latest_intent:
        web_q = f"{q} 最新进展 {years}"
        sub_queries.append(SubQuery(id=f"q{len(sub_queries)+1}", question=web_q, tool="web_search", status="pending"))
        sub_queries.append(SubQuery(id=f"q{len(sub_queries)+1}", question=f"{q} 应用案例和行业动态 {year_now}", tool="web_search", status="pending"))
    else:
    ...
    if "agent" in q.lower() or "智能体" in q:
        sub_queries.append(SubQuery(id=f"q{len(sub_queries)+1}", question=f"{q} 多智能体协作 {years}", tool="arxiv", status="pending"))
